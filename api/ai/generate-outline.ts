...
import { generateAIOutlineEngine } from '../../src/server/aiEngine';
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const result = await generateAIOutlineEngine(body);
    return sendJson(res, 200, result);
  } catch (err: any) {
    console.error('API Error in generate-outline:', err);
    return sendError(res, 500, err.message || 'Failed to generate AI outline');
  }
}
