...
import { generateAISEOEngine } from '../../src/server/aiEngine';
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const result = await generateAISEOEngine(body);
    return sendJson(res, 200, result);
  } catch (err: any) {
    console.error('API Error in generate-seo:', err);
    return sendError(res, 500, err.message || 'Failed to generate SEO');
  }
}
